...
const API_VERSION = API_VERSION_RAW.startsWith('v')
  ? API_VERSION_RAW.substring(1)
  : API_VERSION_RAW;
...
export async function getProducts(): Promise<Product[]> {
  ...
  const response = await axios.get(
    `${instanceUrl}/services/data/v${API_VERSION}/query`,
    {
      params: { q: soql },
  ...
  return records.map((r: any) => ({
    ...
    category: r.Category_c__c ?? 'Overig',
  }));
}

export async function getProductByExternalProductId(
  externalProductId: string
